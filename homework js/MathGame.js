let score = { correct: 0, total:0, incorrect:0, checklevel:0 };
...
const MathLevel = (QuanlityQuestion) =>{
    if (QuanlityQuestion >= 0 && QuanlityQuestion <= 10) {
        Level.innerText = `Level: beginner.`;
    } else if (QuanlityQuestion >= 11 && QuanlityQuestion <= 20) {
        Level.innerText = `Level: intermediate.`;
    } else if (QuanlityQuestion >= 21) {
...
document.getElementById('quanlitytotal').innerHTML=`Total: ${score.total}/30`;
document.getElementById('checkresult').addEventListener('click', function () {
    ...
    else if(score.total>10 && score.total<=20)
    {
        ...
        clearInterval(countdownInterval);
        Calculate.innerText = `Congratulations you've completed it! Result: ${score.correct}/30`;
        document.getElementById('checkresult').disabled = true; // Vô hiệu hóa nút kiểm tra
    }
});
...
function resetGame() {
    ...
    document.getElementById('quanlitytotal').innerHTML = `Total: 0/30`;
